...
// Configured for mail.routing.net
const transporter = nodemailer.createTransport({
  host: process.env.EMAIL_HOST || 'mail.routing.net',
  port: parseInt(process.env.EMAIL_PORT || '587'),
  secure: parseInt(process.env.EMAIL_PORT || '587') === 465, // true for 465, false for other ports
  requireTLS: parseInt(process.env.EMAIL_PORT || '587') === 587, // Require TLS for port 587
  auth: {
    user: process.env.EMAIL_USER,
    pass: process.env.EMAIL_PASS,
  },
  tls: {
    rejectUnauthorized: false // Some mail servers use self-signed certificates
  },
