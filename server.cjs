...
const { createVercelHandlerForward } = require('./api/_lib/server-cjs-vercel-forward.cjs');
...
/** @deprecated P0 login routes delegate to Vercel handlers with shared rate-limit — non-P0 only */
...
/** @deprecated P0 send-email delegates to misc.js (EMAIL_SEND) — unused on P0 path */
...
/** @deprecated P0 scanner-login delegates to api/scan.js (LOGIN_SCANNER) */
...
/** @deprecated P0 orders/create delegates to api/orders-create.js (ORDER_CREATE) */
...
// POST /api/scanner-logout// POST /api/scanner-logout
...
// ============================================
// GET /api/sms-balance - Check WinSMS Account Balance// GET /api/sms-balance - Check WinSMS Account Balance
// ============================================
...
// POST /api/bulk-phones// POST /api/bulk-phones - Add bulk phone numbers
...
// Get email delivery logs for an order (admin only)// Get email delivery logs for an order (admin only)
...
function buildTicketEmailHtml(order, tickets, passes, orderId) {
  ...
  const totalAmount = order.total_with_fees ?? order.total_price ?? 0;
  ...
  return buildOnlineTicketEmailHtml({
...
/** @deprecated P0 resend routes delegate to misc.js (EMAIL_RESEND_TICKET) */
