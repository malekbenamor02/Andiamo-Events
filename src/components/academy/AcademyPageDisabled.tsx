...
export default function AcademyPageDisabled({
  language,
  message,
  soldOut = false,
}: AcademyPageDisabledProps) {
  const title = soldOut
    ? language === 'en'
      ? 'Academy sold out'
      : 'Academy complet'
  ...
  const Icon = soldOut ? TicketX : GraduationCap;
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4 py-16">
      <div className="max-w-lg w-full text-center rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm p-10 shadow-lg">
        {soldOut && (
          <span className="inline-block mb-4 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide bg-primary/15 text-primary border border-primary/30">
            {language === 'en' ? 'Sold out' : 'Complet'}
          </span>
        )}
        <Icon className="w-14 h-14 text-primary mx-auto mb-6 opacity-90" aria-hidden />
