-- Atomic admin ambassador reassignment: order update + admin_reassigned audit log in one transaction.
...
BEGIN
  ...
  v_details := jsonb_strip_nulls(jsonb_build_object(
    ...
    'notify_ambassador', COALESCE(p_notify_ambassador, true),
    'notification_status', CASE
      WHEN COALESCE(p_notify_ambassador, true) THEN 'pending'
      ELSE 'skipped'
    END,
    'email_sent', NULL,
    'sms_sent', NULL,
