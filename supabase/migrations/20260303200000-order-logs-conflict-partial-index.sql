-- Fix: ON CONFLICT must include the partial index predicate so duplicate
-- status_changed logs (e.g. PENDING_ONLINE -> FAILED on retry) do not violate
-- order_logs_status_change_uniq and the trigger can DO NOTHING.
