/**
 * Server-side branded Excel export for Reports (online, ambassador cash, POS).
 * Built for Node; bundled to api/_lib/reports-excel-export.cjs via esbuild.
 */
import ExcelJS from 'exceljs';
import type { WorksheetProtection } from 'exceljs';
import { PaymentMethod } from '@/lib/constants/orderStatuses';
import { isPaidPosOrder } from '@/lib/orders/orderAnalytics';
import { getOrderReportRevenue, getOrderTicketsAndRevenue } from '@/lib/orders/orderRevenue';

type DateRange = 'ALL_TIME' | 'LAST_30_DAYS' | 'LAST_7_DAYS';
...
export type ReportsExcelRosterRow = { id: string; full_name: string; phone: string };

export type BuildReportsExcelParams = {
  eventId: string | null;
  eventName: string | null;
  dateRange: DateRange;
  language?: 'en' | 'fr';
  orders: any[];
  ambassadorRoster: ReportsExcelRosterRow[];
  eventPassNames?: string[];
  passStockRows?: PassStockSheetRow[] | null;
};
...
function resolvePassTypeColumns(
  eventId: string | null,
  ambassadorOrders: any[],
  eventPassNames: string[] | undefined,
  lang: Lang
): string[] {
...
function resolvePassTypesForStockSheet(
  eventId: string | null,
  onlineOrders: any[],
  ambassadorOrders: any[],
  posOrders: any[],
...
function getWorkbookLockPassword(): string | null {
...
export async function buildReportsExcelBuffer(params: BuildReportsExcelParams): Promise<{
  buffer: Buffer;
  filename: string;
  rowCount: number;
}> {
  ...
  let passStockEventRows: PassStockSheetRow[] | null = params.passStockRows ?? null;
