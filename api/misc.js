...
// Helper function to send SMS via WinSMS API
async function sendSms(phoneNumbers, message, senderId = 'Andiamo') {
  ...
  const formattedNumbers = phoneArray.map(phone => formatPhoneNumber(phone)).filter(phone => phone !== null);
...
const MARKETING_ATTACHMENT_MAX_BYTES = Math.min(
  15 * 1024 * 1024,
  Math.max(1024, parseInt(process.env.MARKETING_ATTACHMENT_MAX_BYTES || '9437184', 10) || 9437184)
);

async function fetchMarketingPosterAttachment(urlStr) {
  const { URL } = await import('url');
  const u = normalizeMarketingHeaderImageUrl(urlStr);
  if (!u) throw new Error('Invalid poster attachment URL');
  const parsed = new URL(u);
  if (parsed.protocol !== 'https:') throw new Error('Poster URL must use https');
  const https = await import('https');
  const chunks = [];
  let total = 0;
  await new Promise((resolve, reject) => {
    const req = https.request(
      {
        ...
        method: 'GET',
        timeout: 28000,
        headers: { 'User-Agent': 'Andiamo-Events-Marketing/1.0' },
      },
      (res) => {
        if (!res.statusCode || res.statusCode >= 400) {
          reject(new Error(`Poster fetch HTTP ${res.statusCode || 'unknown'}`));
          return;
        }
        res.on('data', (chunk) => {
          total += chunk.length;
          if (total > MARKETING_ATTACHMENT_MAX_BYTES) {
            res.destroy();
            reject(new Error('Poster file exceeds size limit'));
          } else {
            chunks.push(chunk);
          }
        });
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Poster fetch timeout'));
    });
    req.on('error', reject);
    req.end();
  });
  ...
  if (seg && /^[a-zA-Z0-9._-]+$/.test(seg) && seg.length <= 200) filename = seg;
  else if (seg) filename = String(seg).slice(0, 80).replace(/[^a-zA-Z0-9._-]/g, '_') || 'poster';
  return { buffer: buf, filename };
}

/** Investor-style delivery: dedicated From + optional BREVO_API_KEY_INVESTORS. */
function isInvestorMarketingSend(campaign) {
  if (!campaign || campaign.type !== 'email') return false;
  if (sanitizeEmailTemplate(campaign.email_template) === 'investor_vanguard') return true;
  return String(campaign.sender_profile || '').trim() === 'investor';
}

function marketingEmailEnvelope(campaign) {
  const defaultFrom = '"Andiamo Events" <contact@andiamoevents.com>';
  if (isInvestorMarketingSend(campaign)) {
    const envFrom = process.env.MARKETING_INVESTOR_FROM && String(process.env.MARKETING_INVESTOR_FROM).trim();
    const from = envFrom || '"Andiamo Events" <investors@andiamoevents.com>';
    const rt = process.env.MARKETING_INVESTOR_REPLY_TO && String(process.env.MARKETING_INVESTOR_REPLY_TO).trim();
    return { from, replyTo: rt || from };
  }
  return { from: defaultFrom, replyTo: defaultFrom };
}

function brevoApiKeyForCampaign(campaign) {
  if (isInvestorMarketingSend(campaign)) {
    const inv = process.env.BREVO_API_KEY_INVESTORS && String(process.env.BREVO_API_KEY_INVESTORS).trim();
    if (inv) return inv;
  }
...
/** Institutional template always uses investor mailbox + Brevo investor key (cannot mix with contact@). */
function sanitizeSenderProfile(raw, emailTemplate) {
  const tpl = sanitizeEmailTemplate(emailTemplate);
  if (tpl === 'investor_vanguard') return 'investor';
...
/** Normalize row shape from PostgREST (snake_case) or accidental camelCase. */
function normalizeMarketingCampaignRow(row) {
  if (!row || typeof row !== 'object') return row;
  const o = { ...row };
  if (o.email_template == null && o.emailTemplate != null) o.email_template = o.emailTemplate;
  if (o.sender_profile == null && o.senderProfile != null) o.sender_profile = o.senderProfile;
  if (o.attach_poster == null && o.attachPoster != null) o.attach_poster = o.attachPoster;
  if (o.poster_attachment_url == null && o.posterAttachmentUrl != null) {
    o.poster_attachment_url = o.posterAttachmentUrl;
  }
  return o;
}
...
async function processMarketingCampaignSendBatch(dbClient, campaignId, options = {}) {
  ...
  const { data: rawCampaign, error: campErr } = await dbClient
    .from('marketing_campaigns')
    .select('*')
    .eq('id', campaignId)
    .single();

  if (campErr || !rawCampaign) {
    return { ok: false, statusCode: 404, error: 'Campaign not found' };
  }
  const campaign = normalizeMarketingCampaignRow(rawCampaign);
  if (campaign.type === 'email') {
    console.log('[marketing-email] send-batch campaign row', {
      id: campaignId,
      email_template: campaign.email_template,
      sender_profile: campaign.sender_profile,
      will_use_investor_html: sanitizeEmailTemplate(campaign.email_template) === 'investor_vanguard',
      will_use_investor_mailbox: isInvestorMarketingSend(campaign),
    });
  }
  ...
  if (
    ...
    console.warn('[marketing-email] attachment will be skipped (fetch failed or empty buffer)', {
      campaign_id: campaignId,
    });
  }
  const investorTpl =
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      if (campaign.type === 'email') {
        ...
        const mailInfo = await sendTransactionalEmail({ getEmailTransporter }, mailPayload);
...
export default async (req, res) => {
  ...
  // Parse query for route dispatch (e.g. sitemap via rewrite)
      ...
      try {
        ...
        if (process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY) {
          ...
          const { data: events, error } = await supabase
            .from('events')
            .select('id, date, event_type, updated_at, is_test, slug')
            .order('date', { ascending: false });
          if (!error && events && events.length > 0) {
            const isProduction = process.env.VERCEL_ENV === 'production' || !process.env.VERCEL_ENV;
            const filtered = isProduction ? events.filter((e) => !e.is_test) : events;
            for (const e of filtered) {
              const legacySlug = `event-${e.id}`;
              const lastmod = e.updated_at && new Date(e.updated_at).toISOString().split('T')[0];
              if (e.event_type === 'gallery') {
                const galleryPathSlug = e.slug && String(e.slug).trim() ? String(e.slug).trim() : legacySlug;
                urls.push({ loc: `${SITE_URL}/gallery/${galleryPathSlug}`, lastmod, changefreq: 'monthly', priority: '0.6' });
              } else {
                const upcomingPathSlug = e.slug && String(e.slug).trim() ? String(e.slug).trim() : legacySlug;
                urls.push({ loc: `${SITE_URL}/event/${upcomingPathSlug}`, lastmod, changefreq: 'weekly', priority: '0.8' });
                urls.push({ loc: `${SITE_URL}/${upcomingPathSlug}`, lastmod, changefreq: 'weekly', priority: '0.85' });
              }
            }
          }
          ...
          if (!careersError && careers && careers.length > 0) {
            for (const c of careers) {
              ...
              urls.push({
                loc: `${SITE_URL}/careers/${encodeURIComponent(String(c.slug).trim())}`,
                lastmod,
                changefreq: 'weekly',
                priority: '0.7',
              });
...
${fallbackUrls.map((u) => `  <url>\n    <loc>${esc(u.loc)}</loc>\n    <changefreq>${esc(u.changefreq)}</changefreq>\n    <priority>${esc(u.priority)}</priority>\n  </url>`).join('\n')}
    ...
    // ============================================
    // GET /llms.txt (via rewrite to /api/misc?route=llms) - AI-readable site guide
    // ============================================
    if ((path === '/api/misc' && queryParams.route === 'llms') || path === '/llms.txt') {
      if (method !== 'GET') {
        res.setHeader('Allow', 'GET');
        return res.status(405).end();
      }
      try {
        const SITE_URL = 'https://www.andiamoevents.com';
        const SITE_DESC =
          "Andiamo Events – Tunisia's youth-driven event label by Born To Lead. Concerts, festivals, cultural events, ticket sales, ambassador program, and certified Event Management training (Andiamo Academy) in Tunis and across Tunisia.";
        const lines = [
          '# Andiamo Events',
          '',
          `> ${SITE_DESC}`,
          ...
          '## Main pages',
          '',
          `- [Home](${SITE_URL}/): Event label homepage — upcoming highlights, brand story`,
          `- [Events](${SITE_URL}/events): Browse all upcoming concerts, parties and festivals in Tunisia`,
          `- [About](${SITE_URL}/about): Who we are — Born To Lead, youth-driven production, mission`,
          `- [Contact](${SITE_URL}/contact): Customer support, partnerships, general inquiries`,
          `- [Terms](${SITE_URL}/terms): Ticket purchase terms and conditions`,
          '',
          '## Tickets',
          '',
          'Each upcoming event has an event detail page (`/event/{slug}`) and a ticket purchase page (`/{slug}`).',
          '',
          '## Andiamo Academy (Event Management training)',
          '',
          `- [Academy overview](${SITE_URL}/academy): Certified in-person Event Management training in Tunis — 20 hours, Essential / Pro / Premium formulas`,
          `- [Register](${SITE_URL}/academy/register): Enrollment and formula selection`,
          `- [Academy terms](${SITE_URL}/academy/terms): Training registration, payment and refund rules`,
          '',
          '## Ambassador program',
          '',
          `- [Become an Ambassador](${SITE_URL}/ambassador): Join the Andiamo ambassador program — represent the brand, earn commissions, promote events in Tunisia`,
          '',
          '## Careers',
          '',
          `- [Careers](${SITE_URL}/careers): Open roles at Andiamo Events and Born To Lead`,
        ];
        ...
        if (process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY) {
          ...
          const { data: events, error: eventsError } = await supabase
            .from('events')
            .select('id, name, description, date, venue, city, event_type, is_test, slug')
            .order('date', { ascending: false });
          if (!eventsError && events && events.length > 0) {
            const filtered = isProduction ? events.filter((e) => !e.is_test) : events;
            const upcoming = filtered.filter((e) => e.event_type !== 'gallery');
            const gallery = filtered.filter((e) => e.event_type === 'gallery');
            if (upcoming.length > 0) {
              lines.push('', '## Upcoming events & tickets', '');
              for (const e of upcoming) {
                const slug = (e.slug && String(e.slug).trim()) || `event-${e.id}`;
                const dateStr = e.date ? String(e.date).split('T')[0] : '';
                const venue = [e.venue, e.city].filter(Boolean).join(', ');
                const meta = [dateStr, venue].filter(Boolean).join(' — ');
                const desc = e.description ? ` ${String(e.description).replace(/\s+/g, ' ').trim().slice(0, 120)}` : '';
                lines.push(
                  `- [${e.name}](${SITE_URL}/event/${slug})${meta ? ` (${meta})` : ''}:${desc} [Buy tickets](${SITE_URL}/${slug})`
                );
              }
            }
            if (gallery.length > 0) {
              lines.push('', '## Past events (gallery)', '');
              for (const e of gallery) {
                const slug = (e.slug && String(e.slug).trim()) || `event-${e.id}`;
                lines.push(`- [${e.name}](${SITE_URL}/gallery/${slug}): Photo gallery and recap`);
              }
            }
          }
          ...
          const { data: careers, error: careersError } = await supabase
            .from('career_domains')
            .select('slug, name, description, job_type')
            .eq('applications_open', true);
          if (!careersError && careers && careers.length > 0) {
            lines.push('', '## Open career positions', '');
            for (const c of careers) {
              if (!c.slug) continue;
              const jobType = c.job_type ? ` (${c.job_type})` : '';
              const desc = c.description ? `: ${String(c.description).replace(/\s+/g, ' ').trim().slice(0, 140)}` : '';
              lines.push(`- [${c.name}${jobType}](${SITE_URL}/careers/${c.slug})${desc}`);
            }
          }
        }
        ...
        lines.push(
          ...
          '## Community',
          '',
          `- [Suggestions](${SITE_URL}/suggestions): Suggest artists, venues or event ideas`,
          '',
          '## Crawler resources',
          '',
          `- [XML Sitemap](${SITE_URL}/sitemap.xml): Full list of public URLs including live events`,
          `- [robots.txt](${SITE_URL}/robots.txt): Crawl rules — public pages allowed; admin, POS and scanner blocked`,
          '',
          '## Brand',
          '',
          '- Also known as: Andiamo, Andiamo Event, Andiamo Experience, Andiamoevent',
          '- Country: Tunisia (Tunis, Sousse and nationwide)',
          '- Instagram: https://www.instagram.com/andiamoevents',
          '- Twitter/X: https://twitter.com/andiamo_events',
        );
        ...
        const SITE_URL = 'https://www.andiamoevents.com';
        const fallback = `# Andiamo Events\n\n> Tunisia's youth-driven event label — concerts, festivals, tickets, academy, ambassadors.\n\n- [Home](${SITE_URL}/)\n- [Events](${SITE_URL}/events)\n- [Academy](${SITE_URL}/academy)\n- [Ambassador](${SITE_URL}/ambassador)\n- [About](${SITE_URL}/about)\n- [Contact](${SITE_URL}/contact)\n- [Sitemap](${SITE_URL}/sitemap.xml)\n`;
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        return res.status(200).send(fallback);
      }
    }
    ...
    // ============================================
    // /api/admin/passes/:id/description (PUT)
    // ============================================
    if (path.includes('/api/admin/passes/') && path.endsWith('/description') && method === 'PUT') {
      try {
        ...
        if (updateError) {
          ...
          return res.status(500).json({
            error: 'Failed to update pass description',
            details: updateError.message
          });
        }
      ...
      try {
        ...
        try {
          ...
          if (existingTickets && existingTickets.length > 0) {
            ...
            // Send email with QR codes (template: api/lib/online-ticket-email-html.cjs; preview: email-templates/previews/)
...
Billets envoyés par email. We Create Memories`;
    ...
    // ============================================
    // GET /api/admin/order-qr-tickets?orderId= — super_admin only (QR images + ticket scan/generation status)
    // ============================================
    if (path === '/api/admin/order-qr-tickets' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        if (authResult.admin?.role !== 'super_admin') {
          return res.status(403).json({
            error: 'Forbidden',
            details: 'Super admin privileges required to view order QR tickets',
            valid: false
          });
        }
    ...
    // ============================================
    // GET /api/admin/logs (Admin: comprehensive logs)
    // ============================================
    if (path === '/api/admin/logs') {
      const { handleAdminLogs } = await import('./_lib/admin-logs-route.js');
    ...
    // ============================================
    if (path === '/api/admin/phone-numbers/counts' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        if (requestedSources.includes('ambassador_applications')) {
          ...
          const { count: withPhone } = await dbClient
            .from('ambassador_applications')
            .select('*', { count: 'exact', head: true })
    ...
    // ============================================
    // GET /api/admin/email-addresses/counts - Get quick counts per source (email parity with phone)
    // ============================================
    if (path === '/api/admin/email-addresses/counts' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        if (true) {
          const { count: total } = await dbClient.from('orders').select('*', { count: 'exact', head: true });
          const { count: withEmail } = await dbClient.from('orders').select('*', { count: 'exact', head: true }).not('user_email', 'is', null);
          const { data: cityData } = await dbClient.from('orders').select('city').not('user_email', 'is', null);
          const byCity = {};
          if (cityData) {
            cityData.forEach(item => {
              if (item.city) {
          ...
          counts.orders = { total: total || 0, withEmail: withEmail || 0, byCity };
        }
        ...
        counts.approved_ambassadors = { total: ambTotal || 0, withEmail: ambWithEmail || 0 };

        const { data: appStatusData } = await dbClient.from('ambassador_applications').select('status').not('email', 'is', null);
        const byStatus = {};
        if (appStatusData) {
        ...
        const { count: appTotal } = await dbClient.from('ambassador_applications').select('*', { count: 'exact', head: true });
        const { count: appWithEmail } = await dbClient.from('ambassador_applications').select('*', { count: 'exact', head: true }).not('email', 'is', null);
        counts.ambassador_applications = { total: appTotal || 0, withEmail: appWithEmail || 0, byStatus };

        const { count: aioTotal } = await dbClient.from('aio_events_submissions').select('*', { count: 'exact', head: true });
        const { count: aioWithEmail } = await dbClient.from('aio_events_submissions').select('*', { count: 'exact', head: true }).not('email', 'is', null);
        counts.aio_events_submissions = { total: aioTotal || 0, withEmail: aioWithEmail || 0 };

        const invTotalRes = await dbClient.from('investor_contacts').select('*', { count: 'exact', head: true });
        const invWithRes = await dbClient.from('investor_contacts').select('*', { count: 'exact', head: true }).not('email', 'is', null);
        if (!invTotalRes.error && !invWithRes.error) {
          counts.investors = { total: invTotalRes.count || 0, withEmail: invWithRes.count || 0 };
        } else {
          counts.investors = { total: 0, withEmail: 0 };
        }
    ...
    // ============================================
    // GET /api/admin/email-addresses/sources - Get emails from selected sources with filters (deduplicated)
    // ============================================
    if (path === '/api/admin/email-addresses/sources' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        const { sources, includeMetadata } = queryParams;
        if (!sources) {
        ...
        const deduplicateEmails = (items) => {
          const seen = new Map();
          const duplicates = [];
          items.forEach(item => {
            const norm = normalizeEmail(item.email);
            if (!norm) return;
            if (seen.has(norm)) {
              const existing = seen.get(norm);
              duplicates.push({ email: norm, sources: [existing.source, item.source] });
            } else {
              seen.set(norm, item);
            }
          });
        ...
        if (sourcesConfig.orders?.enabled) {
          ...
          try {
            ...
            const emails = data.filter(o => o.user_email).map(o => ({
              ...
              city: o.city || null,
              ville: o.ville || null,
              metadata: includeMetadata === 'true' ? { order_number: o.order_number, user_name: o.user_name, status: o.status, payment_method: o.payment_method, source: o.source } : undefined
            })).filter(p => p.email);
        ...
        if (sourcesConfig.approved_ambassadors?.enabled) {
          let query = dbClient.from('ambassadors').select('id, email, city, ville, full_name, status').eq('status', 'approved').not('email', 'is', null);
          ...
          if (!error && data) {
            const emails = data.filter(a => a.email).map(a => ({
              email: normalizeEmail(a.email),
              source: 'approved_ambassadors',
              sourceId: a.id,
              city: a.city || null,
              ville: a.ville || null,
              metadata: includeMetadata === 'true' ? { full_name: a.full_name, status: a.status } : undefined
            })).filter(p => p.email);
            allEmails.push(...emails);
            sourceCounts.approved_ambassadors = emails.length;
          }
        }

        if (sourcesConfig.ambassador_applications?.enabled) {
          let query = dbClient.from('ambassador_applications').select('id, email, city, ville, status, full_name').not('email', 'is', null);
          ...
          if (!error && data) {
            const emails = data.filter(a => a.email).map(a => ({
              ...
              city: a.city || null,
              ville: a.ville || null,
              metadata: includeMetadata === 'true' ? { status: a.status, full_name: a.full_name } : undefined
            })).filter(p => p.email);
            allEmails.push(...emails);
            sourceCounts.ambassador_applications = emails.length;
          }
        }

        if (sourcesConfig.aio_events_submissions?.enabled) {
          let query = dbClient.from('aio_events_submissions').select('id, email, city, ville, status, full_name, event_id').not('email', 'is', null);
          ...
          if (!error && data) {
            const emails = data.filter(s => s.email).map(s => ({
              ...
              city: s.city || null,
              ville: s.ville || null,
              metadata: includeMetadata === 'true' ? { status: s.status, full_name: s.full_name, event_id: s.event_id } : undefined
            })).filter(p => p.email);
            allEmails.push(...emails);
            sourceCounts.aio_events_submissions = emails.length;
          }
        }

        if (sourcesConfig.investors?.enabled) {
          const { data, error } = await dbClient.from('investor_contacts').select('id, email, created_at').not('email', 'is', null);
          if (!error && data) {
            const emails = data
              .filter((r) => r.email)
              .map((r) => ({
                ...
                city: null,
                ville: null,
                metadata: includeMetadata === 'true' ? { created_at: r.created_at } : undefined,
              }))
              .filter((p) => p.email);
            allEmails.push(...emails);
            sourceCounts.investors = emails.length;
          }
        }

        const { unique, duplicates } = deduplicateEmails(allEmails);
        ...
        return res.status(500).json({
          success: false,
          error: error.message || 'Failed to fetch email addresses'
        });
      }
    }

    // ============================================
    // GET /api/admin/investor-contacts — list saved investor emails for marketing source
    // ============================================
    if (path === '/api/admin/investor-contacts' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        return res.json({ success: true, data: { contacts: data || [] } });
      } catch (err) {
        console.error('Error investor-contacts GET:', err);
        return res.status(500).json({ success: false, error: err.message });
      }
    }

    // ============================================
    // PUT /api/admin/investor-contacts — replace list (normalized, deduped)
    // ============================================
    if (path === '/api/admin/investor-contacts' && method === 'PUT') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        const bodyData = await parseBody(req);
        const rawList = Array.isArray(bodyData.emails) ? bodyData.emails : [];
        const seen = new Set();
        const emails = [];
        for (const raw of rawList) {
          const v = normalizeCampaignEmailForMarketing(typeof raw === 'string' ? raw : String(raw));
          if (v && !seen.has(v)) {
            seen.add(v);
            emails.push(v);
          }
        ...
        const dbClient = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
        for (;;) {
          const { data: chunk } = await dbClient.from('investor_contacts').select('id').limit(400);
          if (!chunk || chunk.length === 0) break;
          const ids = chunk.map((r) => r.id);
          await dbClient.from('investor_contacts').delete().in('id', ids);
        }
        if (emails.length) {
          const rows = emails.map((e) => ({ email: e }));
          const batch = 400;
          for (let i = 0; i < rows.length; i += batch) {
            const slice = rows.slice(i, i + batch);
            const { error: insErr } = await dbClient.from('investor_contacts').insert(slice);
            if (insErr) {
              return res.status(500).json({ success: false, error: insErr.message || 'Insert failed' });
            }
          }
        }
        return res.json({ success: true, data: { count: emails.length } });
      } catch (err) {
        console.error('Error investor-contacts PUT:', err);
        return res.status(500).json({ success: false, error: err.message });
      }
    }
    ...
    // ============================================
    if (path === '/api/admin/phone-numbers/sources' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
    ...
    // ============================================
    if (path === '/api/admin/bulk-sms/send' && method === 'POST') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        const bodyData = await parseBody(req);
        const { phoneNumbers, message, sources, filters, metadata } = bodyData;
        ...
        for (const phoneData of phoneNumbers) {
          ...
          if (!formattedNumber) {
            ...
            const { data: logData } = await dbClient.from('sms_logs').insert({
              phone_number: phoneNumber,
          ...
          data: {
            ...
            campaign_snapshot: snap
              ? {
                  ...
                  sender_profile: snap.sender_profile,
                  is_investor_send: isInvestorMarketingSend(snap),
                }
              : null,
          },
        });
    ...
    if (path === '/api/marketing/campaigns' && method === 'POST') {
      try {
        ...
        if (type === 'email' && String(mode || '').trim().toLowerCase() === 'draft') {
          ...
          const emailTemplate = sanitizeEmailTemplate(bodyData.email_template);
          const senderProfile = sanitizeSenderProfile(bodyData.sender_profile, emailTemplate);
          const attachPoster = Boolean(bodyData.attach_poster);
          ...
          const insertPayload = {
            ...
            poster_attachment_url: posterAttachmentUrl,
          ...
          console.log('[marketing] draft created', {
          ...
          return res.json({
            success: true,
            data: { campaign_id: campaign.id, status: 'draft', campaign: normalizeMarketingCampaignRow(campaign) },
          });
        }
        ...
        const normalizeCampaignPhone = (p) => {
          if (!p) return null;
          let c = String(p).trim().replace(/\D/g, '');
          if (c.startsWith('00216')) c = c.substring(5);
          else if (c.startsWith('216')) c = c.substring(3);
        ...
        if (campError || !campaign) {
          ...
          return res.status(500).json({ success: false, error: campError?.message || 'Failed to create campaign' });
        }

        const recipientType = type === 'email' ? 'email' : 'phone';
        const recipients = [];
        ...
        if (useExplicitRecipients) {
          ...
          if (recipients.length === 0) {
            return res.status(400).json({
              success: false,
              error: type === 'email' ? 'No valid email addresses in the list' : 'No valid phone numbers in the list'
            });
          }
        } else if (type === 'sms') {
          const allPhones = [];
          if (sourcesConfig.ambassador_applications?.enabled) {
            let q = dbClient.from('ambassador_applications').select('id, phone_number').not('phone_number', 'is', null);
          ...
          if (sourcesConfig.aio_events_submissions?.enabled) {
            let q = dbClient.from('aio_events_submissions').select('id, phone').not('phone', 'is', null);
            ...
            if (f.event_id) q = q.eq('event_id', f.event_id);
            const { data: d } = await q;
            if (d) allPhones.push(...d.map(r => ({ value: normalizeCampaignPhone(r.phone) })).filter(r => r.value));
          }
          if (sourcesConfig.approved_ambassadors?.enabled) {
            let q = dbClient.from('ambassadors').select('id, phone').eq('status', 'approved').not('phone', 'is', null);
          ...
          allPhones.forEach(p => {
            if (p.value && !seen.has(p.value)) {
              seen.add(p.value);
              recipients.push({ recipient_type: 'phone', recipient_value: p.value });
            }
          });
        } else {
          if (!hasEnabledMarketingEmailSource(sourcesConfig)) {
        ...
        if (recipients.length === 0) {
          await dbClient.from('marketing_campaigns').update({ status: 'completed' }).eq('id', campaign.id);
          return res.status(400).json({
            success: false,
        ...
        const { error: insError } = await dbClient.from('marketing_campaign_recipients').insert(rows);
        if (insError) {
          console.error('Failed to insert campaign recipients:', insError);
          return res.status(500).json({ success: false, error: insError.message || 'Failed to save recipients' });
        }
    ...
    // ============================================
    // GET /api/marketing/campaigns - List campaigns with counts
    // ============================================
    if (path === '/api/marketing/campaigns' && method === 'GET') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
        ...
        const { data: campaigns, error } = await dbClient
          .from('marketing_campaigns')
          .select(
            'id, type, name, subject, body, status, batch_size, period, created_at, delay_minutes, batch_delay_minutes, header_image_url, daily_email_cap, cta_url, cta_label, email_template, attach_poster, poster_attachment_url, sender_profile'
          )
          .order('created_at', { ascending: false })
          .limit(100);
    ...
    if (path.match(/^\/api\/marketing\/campaigns\/[^/]+$/) && method === 'GET') {
      try {
        ...
        const { data: c, error } = await dbClient
          .from('marketing_campaigns')
          .select(
            'id, type, name, subject, body, status, batch_size, period, created_at, updated_at, delay_minutes, batch_delay_minutes, header_image_url, daily_email_cap, cta_url, cta_label, email_template, attach_poster, poster_attachment_url, sender_profile'
          )
          .eq('id', campaignId)
          .single();
    ...
    // ============================================
    // POST /api/marketing/campaigns/:id/send-batch - Send next batch
    // ============================================
    if (path.match(/^\/api\/marketing\/campaigns\/[^/]+\/send-batch$/) && method === 'POST') {
      try {
        const authResult = await verifyAdminAuth(req);
        if (!authResult.valid) {
          return res.status(authResult.statusCode || 401).json({
      ...
      try {
        ...
        if (hasContentPatch) {
          ...
          const { data: current, error: curErr } = await dbClient
            .from('marketing_campaigns')
            .select(
              'subject, body, name, header_image_url, cta_url, cta_label, email_template, attach_poster, poster_attachment_url, sender_profile'
            )
            .eq('id', campaignId)
            .single();
          if (curErr || !current) {
          ...
          if (bodyData.email_template !== undefined) {
            patch.email_template = sanitizeEmailTemplate(bodyData.email_template);
          }
          if (bodyData.sender_profile !== undefined) {
            const tpl = patch.email_template !== undefined ? patch.email_template : current.email_template;
            patch.sender_profile = sanitizeSenderProfile(bodyData.sender_profile, tpl);
          }
          if (bodyData.attach_poster !== undefined) {
            patch.attach_poster = Boolean(bodyData.attach_poster);
            if (!patch.attach_poster) patch.poster_attachment_url = null;
          }
          if (bodyData.poster_attachment_url !== undefined) {
            const ap =
              patch.attach_poster !== undefined ? patch.attach_poster : current.attach_poster;
            if (ap) {
              patch.poster_attachment_url = normalizeMarketingHeaderImageUrl(bodyData.poster_attachment_url);
            } else if (patch.attach_poster === undefined) {
              patch.poster_attachment_url = null;
            }
          }
          const effectiveTemplate =
            patch.email_template !== undefined ? patch.email_template : current.email_template;
          if (sanitizeEmailTemplate(effectiveTemplate) === 'investor_vanguard') {
            patch.sender_profile = 'investor';
          }
          ...
          const draftSelect =
            'id, type, name, subject, body, status, batch_size, period, delay_minutes, batch_delay_minutes, header_image_url, daily_email_cap, cta_url, cta_label, email_template, attach_poster, poster_attachment_url, sender_profile, created_at, updated_at';
